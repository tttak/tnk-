...
#if defined(EVAL_NN)
#include "../eval/nn/evaluate_nn_learner.h"
...
#endif
		{
			...
			else
			{
...
#else
				{
					...
					Eval::NN::UpdateParameters(epoch);
				}
#endif
				++epoch;
...
//			dbg_hit_on(false);
		...
		auto pos_add_grad = [&]() {
			...
			Eval::NN::AddExample(pos, rootColor, ps, example_weight);
#endif
		...
		if (newbob_decay != 1.0 && latest_loss_count > 0) {
			...
			if (latest_loss < best_loss) {
				...
				if (best_nn_directory.empty()) {
					...
					Eval::NN::RestoreParameters(best_nn_directory);
				}
				if (--trials > 0 && !is_final) {
					...
					Eval::NN::SetGlobalLearningRateScale(newbob_scale);
				}
			}
			if (trials == 0) {
				cout << "converged" << endl;
...
void learn(Position&, istringstream& is)
{
	...
	Eval::NN::InitializeTraining(eta1,eta1_epoch,eta2,eta2_epoch,eta3);
	Eval::NN::SetBatchSize(nn_batch_size);
	Eval::NN::SetOptions(nn_options);
