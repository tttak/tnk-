...
void load_eval() {
...
#endif
  {
    ...
    const bool result = NN::ReadParameters(stream);
    ASSERT(result);
  }
}
...
Value compute_eval(const Position& pos) {
  return NN::ComputeScore(pos, true);
}
...
Value evaluate(const Position& pos) {
  ...
  Value score = NN::ComputeScore(pos);
#if defined(USE_EVAL_HASH)
...
void evaluate_with_no_return(const Position& pos) {
  NN::UpdateAccumulatorIfPossible(pos);
}
