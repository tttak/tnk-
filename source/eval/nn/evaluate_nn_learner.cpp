...
void RestoreParameters(const std::string& dir_name) {
  const std::string file_name = path_combine(dir_name, NN::kFileName);
...
void save_eval(std::string dir_name) {
  ...
  if (Options["SkipLoadingEval"] && NN::trainer) {
    NN::SendMessages({{"clear_unobserved_feature_weights"}});
  }

  const std::string file_name = path_combine(eval_dir, NN::kFileName);
  std::ofstream stream(file_name, std::ios::binary);
  const bool result = NN::WriteParameters(stream);
  ASSERT(result);
...
// 現在のetaを取得する
double get_eta() {
  return NN::GetGlobalLearningRateScale() * EvalLearningTools::Weight::eta;
}
